...
async def extract_back(request: Request, image: UploadFile = File(...)):
    ...
    try:
        ...
        result_with_pv = await llm.process_task_async(
            [PROMPT_TUNISIAN_ID_BACK, resized_img],
            TunisianIDCardBack
        )
        ...
        try:
            save_pv(indicator="tunisian_id_back", pv=result_with_pv["pv"], save_dir=PV_PATH)
        ...
        raise  # Re-raise known FastAPI errors directly

    except Exception as e:
        logger.error(f"[SERVER] Unexpected error in /back: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
